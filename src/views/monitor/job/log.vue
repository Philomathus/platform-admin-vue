...
<!--      </el-col>-->
    ...
    <!-- 调度日志详细 -->
    <el-dialog v-dialogDrag :close-on-click-modal="false" :title="调度日志详细" :visible.sync="open" width="700px"
               append-to-body>
...
<script>
import {listJobLog, delJobLog, exportJobLog, cleanJobLog} from "@/api/platform-web/monitor/jobLog";
  ...
  methods: {
    ...
    handleExport() {
      ...
      this.$confirm("是否确认导出所有调度日志数据项?", "警告", {
        ...
        this.downloadExcel(response, '调度日志')
      }).catch(() => {
      })
    }
  }
