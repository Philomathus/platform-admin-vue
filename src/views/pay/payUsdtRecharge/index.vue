...
<!--        </template>-->
    ...
    <el-dialog v-dialogDrag :close-on-click-modal="false" :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="充值U数量" prop="rechargeNumber">
          <el-input v-model="form.rechargeNumber" placeholder="请输入充值U数量" />
        </el-form-item>
        <el-form-item label="充值金额" prop="rechargeMoney">
          <el-input v-model="form.rechargeMoney" placeholder="请输入充值金额" />
        </el-form-item>
        <el-form-item label="状态(0已提交1通过2驳回)">
...
<script>
import { listPayUsdtRecharge, getPayUsdtRecharge, delPayUsdtRecharge, addPayUsdtRecharge, updatePayUsdtRecharge, exportPayUsdtRecharge } from "@/api/platform-web/pay/payUsdtRecharge";
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updatePayUsdtRecharge(this.form).then(response => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addPayUsdtRecharge(this.form).then(response => {
