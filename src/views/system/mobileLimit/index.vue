...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listMobileLimit(this.queryParams).then(response => {
        this.mobileLimitList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getMobileLimit(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateMobileLimit(this.form).then(response => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addMobileLimit(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除手机号限制编号为"' + ids + '"的数据项?', "警告", {
