  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="皮肤转盘剩余次数" prop="skinTimes">
        <el-input
          v-model="queryParams.skinTimes"
          placeholder="请输入皮肤转盘剩余次数"
          clearable
          size="small"
          @keyup.enter.native="handleQuery"
        />
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listWheelUser(this.queryParams).then(response => {
        this.wheelUserList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateWheelUser(this.form).then(response => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addWheelUser(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除转盘用户编号为"' + ids + '"的数据项?', "警告", {
